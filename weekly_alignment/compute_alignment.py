"""
Crowd–POI alignment (Ridge): weekly mean of y_pred_mean vs supply-only fit.

Defaults in this copy target Jul–Sep 2025 and write outputs into weekly_alignment/.
Upstream inputs remain under POI_Alignment_0429/ (predictions + POI parquet).
"""
...
def main() -> None:
    ...
    poi_dir = repo_root / "POI_Alignment_0429"

    parser = argparse.ArgumentParser(description="Ridge alignment: mean y_pred vs POI counts.")
    parser.add_argument(
        "--pred-csv",
        type=str,
        default=str(poi_dir / "panel_pred_test_2025_by_date.csv"),
        help="Long table: grid_id, date, y_pred_mean (and optional y_true).",
    )
    parser.add_argument(
        "--poi-parquet",
        type=str,
        default=str(poi_dir / "grid100_poi_static_2024.parquet"),
        help="Static POI features keyed by grid_id.",
    )
    parser.add_argument("--date-start", type=str, default="2025-07-01")
    parser.add_argument("--date-end", type=str, default="2025-09-30")
    parser.add_argument(
        "--pred-col",
        type=str,
        default="y_pred_mean",
    ...
    parser.add_argument(
        "--out-csv",
        type=str,
        default=str(here / "alignment_jul_sep_2025.csv"),
        help="Per-grid alignment table.",
    )
    parser.add_argument(
        "--out-coefs-json",
        type=str,
        default=str(here / "alignment_ridge_coefs_jul_sep_2025.json"),
    ...
    if not pred_path.exists():
        raise SystemExit(
            ...
            "Export with panel_training_0426/export_panel_predictions.py; ensure "
            "panel_pred_test_2025_by_date.csv is under POI_Alignment_0429, or pass --pred-csv."
        )
