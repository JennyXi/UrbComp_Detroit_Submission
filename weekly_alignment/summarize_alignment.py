...
Default paths target Jul–Sep 2025 outputs in weekly_alignment/.
"""
...
def main() -> None:
    ...
    parser.add_argument(
        "--out-summary-json",
        type=str,
        default=str(here / "alignment_summary_jul_sep_2025.json"),
    )
    parser.add_argument(
        "--out-top-positive-csv",
        type=str,
        default=str(here / "alignment_top_positive_jul_sep_2025.csv"),
    )
    parser.add_argument(
        "--out-top-negative-csv",
        type=str,
        default=str(here / "alignment_top_negative_jul_sep_2025.csv"),
    )
    parser.add_argument(
        "--out-priority-csv",
        type=str,
        default=str(here / "alignment_priority_candidates_jul_sep_2025.csv"),
    )
    ...
    if not out_top_pos.is_absolute():
        out_top_pos = (repo_root / out_top_pos).resolve()
    if not out_top_neg.is_absolute():
        out_top_neg = (repo_root / out_top_neg).resolve()
    if not out_priority.is_absolute():
        out_priority = (repo_root / out_priority).resolve()
    ...
    if not high.empty:
        ...
        scarcity_abs_cols = [f"{c}_scarcity_abs" for c in POI_COUNT_COLS]
        scarcity_q_cols = [f"{c}_scarcity_q" for c in POI_COUNT_COLS]
        keep_cols = [
            "grid_id",
            "c_bar",
            "c_hat",
